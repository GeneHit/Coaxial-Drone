...
void OLED_ShowNum_withsymbol(u8 x,u8 y,float num,u8 len,u8 size)
{
		if(num < 0)
		{	
			num = -num;
...
void OLED_ShowFloat(u8 x,u8 y,float number,u8 len,u8 size)
{
	u32 num;
	u32 point;
	if(number >= 0)
	{
		num = (u16)number;
		point = (number * 100 - num * 100);
		OLED_ShowString (x,y," ");
		OLED_ShowNum (x+6,y,num,3,size);
		OLED_ShowString (x+23,y,".");
		OLED_ShowNum (x+26,y,point/10,1,size);
		OLED_ShowNum (x+32,y,point%10,1,size);
	}
	else
	{
		number = -number;
